...
root.innerHTML = `
  ...
  <main id="main" class="legal">
    ...
    ${privacy ? `
      ...
      <h2>Data stored on your device</h2><p>Your rules, schedule, and commitment settings are saved in your browser’s IndexedDB. In the Android APK, the active filtering settings are also copied into Quietwall’s private app storage so the VPN can work while the screen is closed. Quietwall does not store DNS requests, domain lookups, IP addresses, or browsing history. Export creates a JSON file only when you request one.</p>
      <h2>Network access</h2><p>The PWA fetches its own files from this website during installation and updates. Once installed, its app shell works offline. In the APK, permitted DNS requests are relayed directly to the DNS resolver supplied by your current Wi-Fi or mobile network; Quietwall does not receive, proxy, log, or sell that traffic. The app makes no analytics, telemetry, crash-reporting, advertising, or remote API requests.</p>
      ...
      <h2>Known limitations</h2><p>Android permits only one active VPN, so starting another VPN replaces Quietwall. Android Private DNS, an app using its own resolver (including DoH/DoT), direct IP links, and uninstalling Quietwall can bypass DNS filtering. The local DNS VPN supports normal UDP DNS from Android’s configured resolver path; TCP-only and encrypted DNS are not interception paths. The web configurator does not itself intercept traffic. Do not rely on Quietwall for safety-critical filtering or supervision.</p>
