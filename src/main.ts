...
function demoSnapshot(): string {
  return `<div class="demo-snapshot" aria-label="Sample block list summary"><div class="demo-snapshot-meta"><strong>Sample ready</strong><span>Focus hours ${state.scheduleStart}–${state.scheduleEnd} · Pause delay ${state.unlockDelayMinutes} minutes</span><span class="demo-free">Free to use. No account.</span></div><ul>${state.rules.map((rule) => `<li><span class="demo-rule-state" aria-hidden="true">${rule.enabled ? '■' : '□'}</span><span>${escapeHtml(rule.pattern)}</span><strong>${rule.enabled ? 'Enabled' : 'Paused'}</strong></li>`).join('')}</ul></div>`;
}

function workbench(demoPrimary = false): string {
  ...
  const heading = demoPrimary
    ? '<h1 id="page-title" tabindex="-1">Try a sample Android block list</h1>'
    : '<h2 id="workbench-title">Add domains for Android to block</h2>';
  return `<section class="workbench ${demoPrimary ? 'demo-workbench' : ''}" id="configure" aria-labelledby="${demoPrimary ? 'page-title' : 'workbench-title'}"><div class="workbench-heading"><div><p class="eyebrow">${demoPrimary ? 'ISOLATED DEMO' : 'SET UP YOUR BLOCK LIST'}</p>${heading}</div><div class="save-state ${storageError ? 'danger' : ''}" aria-live="polite">${storageError ? '⚠ STORAGE ERROR' : demoMode ? '● DEMO STORAGE' : '● SAVED ON THIS DEVICE'}</div></div>${demoPrimary ? demoSnapshot() : ''}
...
async function boot(): Promise<void> {
  ...
  renderRoute();
  if (demoMode) requestAnimationFrame(() => { document.querySelector<HTMLElement>('#page-title')?.focus({ preventScroll: true }); scrollTo({ top: 0, behavior: 'auto' }); });
  window.addEventListener('popstate', (event) => { renderRoute(true); requestAnimationFrame(() => scrollTo({ top: Number(event.state?.scrollY ?? 0), behavior: 'auto' })); });
}
